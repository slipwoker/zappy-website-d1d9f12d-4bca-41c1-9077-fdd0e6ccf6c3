document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateField(field) {
    ...
    if (isRequired && value === '') {
      const label = field.closest('label, .form-group, .field-wrapper')
        ? (field.closest('.form-group, .field-wrapper') || field.parentNode).querySelector('label')
        : null;
    ...
    if (value !== '') {
      ...
      if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        showFieldError(field, 'Please enter no more than ' + field.maxLength + ' characters.');
        return false;
      }
    }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (isFormValid) {
        ...
        setTimeout(function () {
          ...
          successMsg.style.cssText = 'padding:12px 16px;background:#c6f6d5;color:#276749;border-radius:6px;margin-top:16px;font-weight:500;';
  ...
  animationStyles.textContent = [
    ...
    '}',
    '.fade-in-element.fade-left {',
    '  transform: translateX(-30px);',
    '}',
    '.fade-in-element.fade-right {',
    '  transform: translateX(30px);',
    '}',
    '.fade-in-element.fade-left.visible,',
    '.fade-in-element.fade-right.visible {',
  ...
  function applyAnimationClasses(elements) {
    elements.forEach(function (el, index) {
      if (el.classList.contains('fade-in-element')) return;

      el.classList.add('fade-in-element');

      const direction = el.getAttribute('data-animate-direction');
      if (direction === 'left') {
      ...
      if (delay) {
        ...
        const siblings = el.parentNode ? el.parentNode.querySelectorAll('.fade-in-element') : [];
        const siblingIndex = Array.prototype.indexOf.call(siblings, el);
  ...
  applyAnimationClasses(animatableElements);

  function checkVisibility() {
    const scrollElements = document.querySelectorAll('.fade-in-element:not(.visible)');
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          if (node.nodeType === 1) {
            const newAnimatables = node.querySelectorAll
              ? node.querySelectorAll(animatableSelectors.join(', '))
              : [];
            applyAnimationClasses(newAnimatables);
            newAnimatables.forEach(function (el) {
              observer.observe(el);
            });
            if (node.matches && node.matches(animatableSelectors.join(', '))) {
              applyAnimationClasses([node]);
    ...
    "translations": {
      "en": {
        "consentModal": {
          ...
          "description": "ColorWave uses cookies to enhance your experience, analyze site usage, and assist in our marketing efforts. You can manage your preferences anytime.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'd1d9f12d-4bca-41c1-9077-fdd0e6ccf6c3',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'd1d9f12d-4bca-41c1-9077-fdd0e6ccf6c3';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd1d9f12d-4bca-41c1-9077-fdd0e6ccf6c3';
